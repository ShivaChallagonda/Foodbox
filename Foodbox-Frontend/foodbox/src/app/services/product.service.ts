...
export class ProductService {
  ...
  private categoryUrl = "http://localhost:8081/api/cuisines";

  private newURL = "http://localhost:8081/api/v1/products";
  ...
  getProductListPaginate(
    ...
    if (sortingMethod == "First Created") {
      ...
      this.getListURL =
        `${this.baseUrl}/search/findByCuisine?cuisine=${theCategoryId}` +
        `&page=${thePage}&size=${thePageSize}`;
    } else if (sortingMethod == "Last Created") {
      this.getListURL =
        `${this.baseUrl}/search/findByCuisineOrderByDateCreatedDesc?cuisine=${theCategoryId}` +
        `&page=${thePage}&size=${thePageSize}`;
    } else {
      this.getListURL =
        `${this.baseUrl}/search/findByCuisineOrderByNameAsc?cuisine=${theCategoryId}` +
        `&page=${thePage}&size=${thePageSize}`;
    }
  ...
  getProductCategories(): Observable<ProductCategory[]> {
    return this.httpClient
      ...
      .pipe(map((response) => response._embedded.cuisines));
  }
}
